...
import {
  AppContext,
  SiteDefaultIcon,
} from "../../services/siteDefaultsService";
import { createSiteStyle } from "../../styles/siteStyleBuilder";

export const NavigationItemComponent: React.FC<NavigationItemComponentProps> =
  ({ siteFeature, displayIcons }: NavigationItemComponentProps) => {
      ...
      <>
        {siteFeature && (
          <ListItem
            button
            key={siteFeature.id}
            className={componentStyle.navListItem}
          >
            <Button
              className={componentStyle.navLinkButton}
              component={Route}
              path={siteFeature.path}
              startIcon={
                displayIcons && (
                  <Icon>
                    {context.featureIconCollection.get(siteFeature.name)
                      ?.fontIcon || SiteDefaultIcon.fontIcon}
                  </Icon>
                )
              }
            >
